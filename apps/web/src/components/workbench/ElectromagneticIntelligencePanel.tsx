...
import { 
  ...
  Compass, Circle, Globe, ChevronRight, Info, Database,
  ...
  Scaling, Anchor, PenTool, Boxes, Hammer, Search, Sparkles, Grid
} from 'lucide-react';
import { useEngineeringStore } from '@/store/useEngineeringStore';

/**
 * @interface EMMetricProps
 * High-fidelity electromagnetic telemetry component.
 */
const EMMetric = ({ label, value, unit, icon: Icon, color = 'blue', status }: any) => (
...
/**
 * ElectromagneticIntelligencePanel v3.2 (Phase 55 Advanced - Sovereign EM Infrastructure)
 * 
 * Advanced electromagnetic orchestration kernel with high-fidelity field telemetry,
 * Maxwell-Poisson solvers, and reality-linked field synchronization.
 * Features FDTD_SOLVER, MAXWELL_SOLVER, and MOM_SOLVER kernels with sub-picowatt residual monitoring.
 */
const ElectromagneticIntelligencePanel = () => {
  ...
  const tabs = useMemo(() => [
    ...
    { id: 'FDTD_SOLVER', label: 'FDTD Solver', icon: Layers },
    { id: 'POISSON_SOLVER', label: 'Poisson Solver', icon: Sparkles },
    { id: 'WAVEGUIDE_SOLVER', label: 'Waveguide Solver', icon: Radio },
    { id: 'ANTENNA_SOLVER', label: 'Antenna Solver', icon: Radio },
    { id: 'RADAR_SOLVER', label: 'Radar Solver', icon: Target },
    { id: 'PLASMA', label: 'Plasma Sync', icon: Flame },
    { id: 'MHD', label: 'MHD Dynamics', icon: Globe },
  ], []);

  const runFieldSolver = (type: string) => {
    pushEvent?.('EM_SOLVER_START', { type, timestamp: Date.now() });
    addNotification?.({
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden animate-in fade-in slide-in-from-right-4 duration-500">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6 custom-scrollbar relative">
        ...
        <div className="p-4 bg-blue-500/5 border border-blue-500/20 rounded-3xl flex items-center justify-between mb-4 shadow-inner group hover:border-blue-400 transition-all">
          <div className="flex items-center gap-3">
            ...
            <div className="flex flex-col">
              <span className="text-[11px] text-blue-100 font-black uppercase tracking-[0.2em]">Electromagnetic Cognition Kernel</span>
              <span className="text-[8px] text-blue-400/40 uppercase font-mono tracking-widest italic">REALTIME_MAXWELL_FIELD_SYNC</span>
            </div>
          </div>
          <div className="flex items-center gap-6 text-[10px] font-mono text-white/40">
            <div className="flex flex-col items-end">
               <span>FLUX_RESIDUAL:</span>
               <span className="text-blue-400 font-black tracking-tighter">&lt; 0.98e-15 W/m²</span>
            </div>
            ...
            <section className="space-y-5">
              <div className="flex justify-between items-center px-1">
                <h3 className="text-[11px] font-black text-[#adc6ff]/60 uppercase tracking-[0.3em] flex items-center gap-3">
                  <Waves className="w-4 h-4 text-blue-400" /> EM Field Telemetry
                </h3>
              ...
              <div className="grid grid-cols-2 gap-6">
                <EMMetric 
                  ...
                  value={emFields.electricField_Vm.toExponential(4)} 
                  unit="V/m" 
                  icon={Activity}
                  color="blue"
                ...
                />
                <EMMetric 
                  label="Magnetic Flux (B)" 
                  value={emFields.magneticFlux_T.toExponential(6)} 
                  unit="T" 
                  icon={Magnet}
                  color="amber"
                  status="GAUSS_VERIFIED"
                />
              </div>
              ...
              <div className="p-8 bg-[#0B0F14] border border-[#adc6ff]/10 rounded-[32px] space-y-10 relative overflow-hidden shadow-2xl group">
                ...
                <div className="flex justify-between items-center relative z-10">
                   <div className="space-y-1">
                     <span className="text-[14px] text-blue-400 font-black uppercase tracking-[0.4em]">Poynting Vector Energy Flux</span>
                     <p className="text-[9px] text-[#adc6ff]/30 uppercase font-mono tracking-widest">WATT_DENSITY_SYNC_ACTIVE_v3.2</p>
                   </div>
                ...
                <div className="space-y-10 relative z-10">
                   <div className="space-y-4">
                      ...
                      <div className="h-2.5 bg-[#adc6ff]/5 rounded-full overflow-hidden shadow-inner border border-white/5">
                         <div className="h-full bg-gradient-to-r from-blue-400 to-blue-600 shadow-[0_0_20px_rgba(96,165,250,0.6)] transition-all duration-1000" style={{ width: '68%' }} />
                      </div>
                   </div>
                </div>
                
                <button 
                  onClick={() => runFieldSolver('POYNTING_FLUX_STABILIZATION')}
                ...
                >
                   <div className="flex items-center gap-5">
                      ...
                      <span className="text-[12px] text-blue-100 font-black uppercase tracking-[0.3em]">Run Maxwell Equilibrium Solver</span>
                   </div>
        ...
        {/* TAB: MAXWELL_SOLVER (Field Equilibrium Sync) */}
          ...
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <section className="space-y-5">
              <h3 className="text-[11px] font-black text-emerald-400/60 uppercase tracking-[0.3em] flex items-center gap-3">
                <Sparkles className="w-4 h-4 text-emerald-400" /> High-Fidelity Maxwell Solver
              </h3>
              <div className="p-10 bg-emerald-500/5 border border-emerald-500/20 rounded-[40px] space-y-10 relative overflow-hidden shadow-2xl group">
                ...
                <div className="flex items-center justify-between relative z-10">
                  <div className="flex items-center gap-6">
                    ...
                    <div className="flex flex-col">
                      <span className="text-[16px] font-black text-white uppercase tracking-[0.4em]">Electrodynamic Potential Sync</span>
                      <span className="text-[10px] text-emerald-400/40 uppercase font-mono font-bold tracking-widest">MAXWELL_COGNITION_LOCKED</span>
                    </div>
                  </div>
                  <div className="flex flex-col items-end">
                    <span className="text-4xl font-mono font-black text-emerald-400">0.9999</span>
                    <span className="text-[9px] text-emerald-400/40 uppercase font-mono">VECTOR_POTENTIAL_RESIDUAL</span>
                  </div>
                </div>
                ...
                <div className="h-56 bg-black/60 rounded-[32px] border border-emerald-500/10 flex flex-col items-center justify-center space-y-6 relative overflow-hidden shadow-inner">
                   ...
                   <p className="text-[14px] font-mono text-emerald-400/80 uppercase animate-pulse tracking-[0.8em] relative z-10">Solving Differential Vector Fields...</p>
                ...
                <button 
                  onClick={() => runFieldSolver('MAXWELL_POTENTIAL_INITIALIZE')}
        ...
        {!['FIELDS', 'FDTD_SOLVER', 'PLASMA', 'MHD', 'POWER', 'RF', 'RADAR_SOLVER', 'ANTENNA_SOLVER', 'WAVEGUIDE_SOLVER', 'MAXWELL_SOLVER', 'POISSON_SOLVER'].includes(activeTab) && (
          <div className="h-full flex flex-col items-center justify-center text-center p-20 space-y-10 opacity-40">
            ...
            <div className="space-y-4">
              ...
              <p className="text-[11px] text-[#adc6ff]/20 uppercase tracking-[0.4em] font-mono font-bold italic">Synchronizing EM State Tensors v3.2</p>
            </div>
          </div>
        )}
      ...
      {/* 3. EM MISSION SYSTEM FOOTER (v3.2 Hardened) */}
      <div className="p-6 bg-[#080B10]/95 border-t border-[#adc6ff]/10 space-y-6 shadow-[0_-10px_40px_rgba(0,0,0,0.5)] backdrop-blur-3xl relative z-20">
        ...
        <div className="flex items-center justify-between">
           <div className="flex items-center gap-4 text-[14px] text-blue-400 font-black uppercase tracking-[0.4em] group cursor-pointer">
              ...
              <div className="flex flex-col">
                 <span>Electromagnetic Cognition Hub</span>
        ...
        <div className="flex gap-4">
          ...
          >
            ...
            <span className="relative z-10">Execute EM Load Balance</span>
          </button>
